    ...
    switch(cdata->command) {
        case H5T_CONV_INIT:
            ...
            if(H5T_REFERENCE != dst->shared->type)
                HGOTO_ERROR(H5E_DATATYPE, H5E_BADTYPE, FAIL, "not a H5T_REFERENCE datatype")
            if(!src->shared->u.atomic.u.r.opaque || !dst->shared->u.atomic.u.r.opaque)
        ...
        case H5T_CONV_CONV:
        {
            ...
            while(nelmts > 0) {
                ...
                for(elmtno = 0; elmtno < safe; elmtno++) {
                    ...
                    if(0 == (buf_size = src->shared->u.atomic.u.r.getsize(src->shared->u.atomic.u.r.f, s, src->shared->size)))
                    ...
                    /* Read reference */
                    if(src->shared->u.atomic.u.r.read(src->shared->u.atomic.u.r.f, s, src->shared->size, conv_buf, buf_size) < 0)
                        HGOTO_ERROR(H5E_DATATYPE, H5E_READERROR, FAIL, "can't read reference data")

                    /* Write reference to destination location */
                    if(dst->shared->u.atomic.u.r.write(dst->shared->u.atomic.u.r.f, conv_buf, buf_size, d, dst->shared->size, b) < 0)
