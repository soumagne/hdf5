...
H5T__init_package(void)
{
    ...
    H5T_t       *objref=NULL;           /* Datatype structure for deprecated reference objects */
                                        /* No datatype structure for deprecated region references */
    ...
    switch(dt1->shared->type) {
        ...
        default:
            ...
            switch (dt1->shared->type) {
                ...
                case H5T_REFERENCE:
//                    if(dt1->shared->u.atomic.u.r.opaque < dt2->shared->u.atomic.u.r.opaque)
//                        HGOTO_DONE(-1);
//                    if(dt1->shared->u.atomic.u.r.opaque > dt2->shared->u.atomic.u.r.opaque)
//                        HGOTO_DONE(1);
...
H5T_set_loc(H5T_t *dt, H5F_t *f, H5T_loc_t loc)
{
    ...
    if(dt->shared->force_conv) {
        ...
        switch(dt->shared->type) {
            ...
            case H5T_REFERENCE:
                /* Opaque reference types go through type conversion */
                if(dt->shared->u.atomic.u.r.opaque) {
                    ...
                    break;
                /* Other reference types go through type conversion */
                else
                    HGOTO_ERROR(H5E_DATATYPE, H5E_BADTYPE, FAIL, "Invalid reference type");
                break;
...
H5T__detect_vlen_ref(const H5T_t *dt)
{
    ...
    /* TODO currently only allows for H5R_OBJECT1 */
    if(H5T_REFERENCE == dt->shared->type && H5R_OBJECT1 != dt->shared->u.atomic.u.r.rtype)
        HGOTO_DONE(TRUE);
